...
--
-- 1.1: the buttons sit above the findings list, and the list shows at most 60
-- rows. An infected M9 card turned up with 373 findings; every row is a focus
-- stop on a keyboard board and the firmware's key navigation collects at most
-- 160 of them per screen, so 1.0's buttons below the list could not be reached
-- on an M9 at all.
...
-- Rows past this are summed up in one "and N more" row (see the 1.1 note above).
local MAX_ROWS = 60

local function fill_list(l, items, label_of)
  local shown = math.min(#items, MAX_ROWS)
  for i = 1, shown do
    local it = items[i]
    l:add(label_of(it), function() sys.toast(it.path, 3000) end)
  end
  if #items > shown then
    l:add(string.format(tr("and %d more"), #items - shown))
  end
end

-- A findings list from y to the bottom of the screen, never shorter than a couple
-- of rows: below that the page scrolls instead.
local function list_below(y)
  local lh = math.max(60, H - M - y)
  ui.scroll(y + lh > H)
  return ui.list(M, y, W - 2 * M, lh)
end
...
local function show_removed()
  ...
  if #S.failed == 0 then
    ...
    _, y = text(string.format(tr("Removed %d, %d could not be removed."), S.removed, #S.failed),
                y, 16, C.bad)
    _, y = text(NOTICE, y, 12, C.text)
    -- Button first, list after: see the 1.1 note at the top.
    ui.button(tr("Scan again"), M, y, 120, 32, function() show_intro() end)
    fill_list(list_below(y + 38), S.failed,
              function(f) return short(f.path, 40) .. "  (" .. f.err .. ")" end)
    return
  end
...
show_results = function()
  ...
  if n == 0 then
    _, y = text(NOTICE, y, 12, C.text)
    ui.button(tr("Scan again"), M, button_row(y), 110, 32, function() show_intro() end)
    return
  end
  if not CAN_CLEAN then
    _, y = text(tr("This firmware can find these but not remove them. Update wadamesh, or format the card: in Files, hold the SD card row."),
                y, 12, C.text)
  end
  -- Buttons first, list after: they are the first focus stops on a keyboard board
  -- whatever the list holds (see the 1.1 note at the top).
  local again_x = M
  if CAN_CLEAN then
    ui.button(tr("Remove all"), M, y, 110, 32, function() show_confirm() end)
    again_x = M + 120
  end
  ui.button(tr("Scan again"), again_x, y, 110, 32, function() show_intro() end)
  fill_list(list_below(y + 38), S.found, function(f) return f.why .. ": " .. short(f.path, 40) end)
end
