...
struct __attribute__((packed)) TouchCfg {
  ...
  uint8_t  ts_on_top;         // channel bubbles: put timestamp on the sender row (bool, 1=on) — v40 (trailing)
};
...
static void cfgSetDefaults(TouchCfg& c) {
  ...
  c.ts_on_top          = 1;     // ON: channel bubble timestamp shares the sender row (shorter bubbles)
}
  ...
  if (s_prefs.isKey(KEY_CFG)) {
    ...
    if (n >= offsetof(TouchCfg, bright) && tmp.magic == TOUCH_CFG_MAGIC) {
      ...
      if (s_cfg.ver < TOUCH_CFG_VER) {
        ...
        if (s_cfg.ver < 40) s_cfg.ts_on_top = 1;          // new trailing field: channel timestamp-on-top ON by default
...
// Channel bubble timestamp placement: ON = share the sender row (shorter bubbles);
// OFF = footer under the body (classic layout). Default ON.
bool touchPrefsGetTimestampOnTop() {
  if (!s_begun) touchPrefsBegin();
  return s_cfg.ts_on_top != 0;
}
bool touchPrefsSetTimestampOnTop(bool on) {
  if (!s_begun) touchPrefsBegin();
  s_cfg.ts_on_top = on ? 1 : 0;
  return cfgFlush();
}
