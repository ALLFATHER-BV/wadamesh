...
struct __attribute__((packed)) TouchCfg {
  ...
  uint8_t  retry_echo;        // auto-retry sends until echo/ACK heard (bool, default OFF/opt-in) — v44, MOVED to the tail at v45
  // ⚠️ APPEND new fields HERE (below every existing one) — NEVER insert mid-struct.
  // The blob is read/written as one raw memcpy, so an inserted field shifts every
  // later byte of an already-stored blob on upgrade. beta_57 inserted retry_echo
  // above web_mirror and every upgrading device booted into remote mode (v45 repair).
};
...
#endif
        // v45 REPAIR: beta_57 (the only writer of ver 44) declared retry_echo BETWEEN
        // rx_queue and web_mirror instead of at the struct tail, so on upgrade the raw
        // blob copy shifted every later u8 by one: remote_mode inherited the forced-1
        // remote_landscape (v37) — devices booted into remote mode — map_tile_debug
        // inherited hist_sync_after's 2 (debug overlay on), and hist_sync_after fell
        // to 0 (blocking chat-flush fallback silently disabled). The pre-beta_57
        // values of this run are unrecoverable from a v44 blob (they were overwritten
        // when v44 re-flushed), so reset the whole shifted run to its defaults.
        if (s_cfg.ver == 44) {
          s_cfg.web_mirror       = 0;
          s_cfg.remote_mode      = 0;
          s_cfg.remote_landscape = 1;
          s_cfg.web_terminal     = 0;
          s_cfg.map_tile_debug   = 0;
          s_cfg.hist_sync_after  = 2;
        }
        // v45: auto-retry is opt-in now (beta_57 shipped it default-ON; the default is
        // the user's choice to make). Also covers v44 blobs — the field moved position.
        if (s_cfg.ver < 45) s_cfg.retry_echo = 0;
