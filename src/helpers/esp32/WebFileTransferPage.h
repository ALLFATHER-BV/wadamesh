...
function controls(){const ready=ws&&ws.readyState===1&&authed&&state==='ready';E('upload').disabled=!(ready&&file);E('refresh').disabled=!ready;E('filter').disabled=!ready;E('cancel').disabled=state!=='uploading'&&state!=='downloading'}
...
function renderFiles(){const box=E('files'),filter=E('filter').value,shown=entries.filter(item=>filter==='all'||fileKind(item.path)===filter);box.textContent='';box.className=shown.length?'':'empty';if(!shown.length){box.textContent=entries.length?'No files match this filter.':'No files found.';return}shown.forEach(item=>{const row=document.createElement('div');row.className='file';const info=document.createElement('span');const title=document.createElement('b');title.textContent=item.path;const size=document.createElement('small');size.textContent=item.size.toLocaleString()+' bytes';info.append(title,size);const actions=document.createElement('div');actions.className='actions';const download=document.createElement('button');download.textContent='Download';download.onclick=()=>startDownload(item.path);const remove=document.createElement('button');remove.className='danger';remove.textContent='Delete';remove.onclick=()=>deleteDeviceFile(item.path);actions.append(download,remove);row.append(info,actions);box.append(row)})}
...
E('connect').onclick=connect;E('upload').onclick=upload;E('cancel').onclick=cancel;E('refresh').onclick=refreshList;E('filter').onchange=renderFiles;E('pick').onchange=e=>choose(e.target.files[0]);
